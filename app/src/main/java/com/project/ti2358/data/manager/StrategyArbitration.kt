...
//    var selectedStocks: MutableList<Stock> = synchronizedList(mutableListOf())
//    var shortStocks: MutableList<Stock> = synchronizedList(mutableListOf())
    ...
    suspend fun resort(long: Boolean): MutableList<Stock> = withContext(StockManager.stockContext) {
        currentSort = if (currentSort == Sorting.DESCENDING) Sorting.ASCENDING else Sorting.DESCENDING
        val temp = stocks.toMutableList()

        temp.removeAll { it.askPriceRU == 0.0 || it.bidPriceRU == 0.0 }

        if (long) {
            temp.removeAll { it.changePriceArbLongPercent <= 0.0 || it.askPriceRU == 0.0 }
            ...
            temp.removeAll { it.short == null || it.changePriceArbShortPercent <= 0.0 || it.bidPriceRU == 0.0 }
