...
class Strategy1000BuyFinishFragment : Fragment() {
    ...
    inner class Item1005RecyclerViewAdapter(
        ...
        fun refreshPercent(holder: ViewHolder) {
            ...
            holder.priceChangePercentView.text = "%.2f".format(percent) + "%"
            holder.priceChangeAbsoluteView.text = "%.2f$".format(item.absoluteLimitPriceChange)
            holder.priceChangeAbsoluteTotalView.text = "%.2f$".format(item.absoluteLimitPriceChange * item.lots)

            holder.priceBuyView.text = "%.2f $".format(item.getLimitPriceDouble())
            holder.priceBuyTotalView.text = "%.2f $".format(item.getLimitPriceDouble() * item.lots)
