...
@KoinApiExtension
class Strategy2225Service : Service() {

    private val NOTIFICATION_ACTION = "event.2225"
    private val NOTIFICATION_CANCEL_ACTION = "event.2225.cancel"
    private val NOTIFICATION_CHANNEL_ID = "2225 CHANNEL NOTIFICATION"
    private val NOTIFICATION_ID = 2225
        ...
        notificationButtonReceiver = object : BroadcastReceiver() {
            override fun onReceive(context: Context, intent: Intent) {
                ...
                if (type == NOTIFICATION_CANCEL_ACTION) {
                    ...
                    context.stopService(Intent(context, Strategy2225Service::class.java))
                }
            }
        }
    ...
    override fun onCreate() {
        super.onCreate()
        val notification = Utils.createNotification(this, NOTIFICATION_CHANNEL_ID, "2225", "", "", "")
    ...
    override fun onDestroy() {
        Toast.makeText(this, "Шорт 2225 отменена", Toast.LENGTH_SHORT).show()
        if (notificationButtonReceiver != null) unregisterReceiver(notificationButtonReceiver)
        notificationButtonReceiver = null
        isServiceRunning = false
        job?.cancel()
        strategy2225.stopStrategy()
        super.onDestroy()
    }

    private fun schedulePurchase() {
        Toast.makeText(this, "Запущен таймер на шорт в 2225", Toast.LENGTH_SHORT).show()
        ...
        if (dayTime.size < 3) {
            Utils.showToastAlert("Неверный формат времени! пример: 22:25:00")
            stopService()
            return
        }
    ...
    private fun stopService() {
        Toast.makeText(this, "2225 остановлена", Toast.LENGTH_SHORT).show()
        try {
            wakeLock?.let {
                if (it.isHeld) {
                    it.release()
    ...
    private fun updateNotification(): Long {
        ...
        if (updateTitle) {
            if (hours + minutes + seconds <= 0) {
                strategy2225.startStrategy()
                updateTitle = false
                title = "Шорт!"
            } else {
                title = "Шорт через %02d:%02d:%02d".format(hours, minutes, seconds)
            }
        }

        val shortText: String = strategy2225.getNotificationTextShort()
        val longText: String = strategy2225.getNotificationTextLong()
        val longTitleText: String = "~" + strategy2225.getTotalPurchaseString() + " ="
